...
def test_non_data_line_is_skipped(tts_clip_module) -> None:
    ev = tts_clip_module.parse_mini_max_sse("event: ping")
    assert ev.kind == "skip"
    assert ev.audio == b""


def test_empty_line_is_skipped(tts_clip_module) -> None:
    assert tts_clip_module.parse_mini_max_sse("").kind == "skip"
    assert tts_clip_module.parse_mini_max_sse("   ").kind == "skip"


def test_invalid_json_is_skipped(tts_clip_module) -> None:
    assert tts_clip_module.parse_mini_max_sse("data: {not json").kind == "skip"
...
def test_progress_delta_is_skipped(tts_clip_module) -> None:
    ...
    assert tts_clip_module.parse_mini_max_sse(_sse(audio="ff", status=1)).kind == "skip"
...
def test_final_chunk_returns_decoded_audio(tts_clip_module) -> None:
    ...
    ev = tts_clip_module.parse_mini_max_sse(_sse(audio=hex_audio, status=2))
...
def test_final_chunk_with_empty_audio_still_finishes(tts_clip_module) -> None:
    ...
    ev = tts_clip_module.parse_mini_max_sse(_sse(audio="", status=2))
    assert ev.kind == "final"
    assert ev.audio == b""


def test_base_resp_error_short_circuits(tts_clip_module) -> None:
    ev = tts_clip_module.parse_mini_max_sse(
...
def test_base_resp_success_status_is_ignored(tts_clip_module) -> None:
    ...
    ev = tts_clip_module.parse_mini_max_sse(_sse(audio="ff", status=1, base_code=0))
    assert ev.kind == "skip"
